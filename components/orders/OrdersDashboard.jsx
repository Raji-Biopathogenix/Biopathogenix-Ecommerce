...
export default function OrdersDashboard({ editOrderId = "" }) {
  ...
  useEffect(() => {
    if (editOrderId && !localStorage.getItem('access_token')) {
      router.replace(`/my-account?next=${encodeURIComponent(`/orders/${editOrderId}/edit`)}`)
      return
    }
    if (searchParams) {
  ...
  }, [searchParams, editOrderId, router])
  ...
  useEffect(() => {
    if (editOrderId && user && /^\d+$/.test(editOrderId)) {
      void refresh({ id: editOrderId })
    }
  }, [editOrderId, user, refresh]);
    ...
    <div style={{ minHeight: "100vh", backgroundColor: "#ffffff", color: "#111111", fontFamily: "sans-serif" }}>
      {orderObj && orderObj?.items?.length > 0 && (
        <AdminOrderEditModal
          ...
          onClose={() => {
            setOrderObj(null)
            if (editOrderId) router.replace('/orders')
          }}
